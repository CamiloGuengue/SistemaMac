...
// for photos
const photosButtons = document.querySelectorAll('.photosButton');

photosButtons.forEach(button => {
    button.addEventListener('click', () => {

        const existing = document.getElementById('photosWindow');
        if (existing) {
            existing.remove();
            return;
        }
        ...
        windowApp.id = 'photosWindow';

        const iframe = document.createElement('iframe');
        iframe.src = 'apps/photos/photos.html';
        iframe.style.width = '100%';
        iframe.style.height = '100%';
        iframe.style.border = 'none';

        windowApp.appendChild(iframe);
        appsOnDisplay.appendChild(windowApp);
    });
});
